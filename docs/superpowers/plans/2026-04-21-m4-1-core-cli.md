...
async fn preflight_only_returns_per_driver_checks() {
    ...
    assert_eq!(report.checks[0].kind, agentenv_proto::DriverKind::Sandbox);
    assert_eq!(report.checks[1].kind, agentenv_proto::DriverKind::Agent);
    assert_eq!(report.checks[2].kind, agentenv_proto::DriverKind::Context);
}
```
...
pub async fn run_preflight_only(
    ...
    let mut checks = vec![
        crate::admission::PreflightCheck {
            kind: agentenv_proto::DriverKind::Sandbox,
        ...
        crate::admission::PreflightCheck {
            kind: agentenv_proto::DriverKind::Context,
    ...
    match (selection.inference.as_ref(), set.inference.as_mut()) {
        (Some(inference_name), Some(inference)) => {
            ...
            checks.push(crate::admission::PreflightCheck {
                kind: agentenv_proto::DriverKind::Inference,
...
Review hardening: `PreflightCheck.driver` should preserve the selected driver identifier, not the initialized driver's canonical name, so CLI output and persisted admission data round-trip with the blueprint/selection. If `selection.inference` is set, the factory must return an inference driver; otherwise preflight returns `RuntimeError::MissingSelectedDriver` instead of accepting with a silently omitted capability.
...
Note after Task 3 review: the implemented runtime API returns `RuntimeSecret` from credential providers, validates initialize handshakes in the `initialize_*_driver` helpers, and makes `component_spec`/`agent_spec`/`context_spec`/`inference_spec` return `RuntimeResult` so config conversion failures are not silently dropped. Later task snippets should use that hardened API shape.
...
- Dot-prefixed registry names are reserved for hidden/transient state and must not be valid env names.
- `list_envs` may skip invalid/hidden registry names, but visible registry directories with unreadable, corrupt, or mismatched state are registry-integrity errors.
- `list_envs` and `describe_env` must use the directory name as the env identity and reject persisted state whose `state.name` differs.
- Registry directories and registry files (`state.json`, `blueprint.yaml`, `lock.yaml`) must be real directories/files; symlinked entries are rejected before reading.
...
```bash
git add crates/agentenv-core/src/env.rs crates/agentenv-core/src/runtime.rs
...
- Runtime command APIs must initialize every driver before calling that driver's command method, preserving the protocol/capability handshake outside the create path.
- `status_env` reports missing driver handles as absent summaries instead of failing, so partially-created envs remain inspectable.
- `destroy_env` must be usable for partially-created envs: skip missing handles, destroy the sandbox first when present, tear down inference/context handles when present, and persist cleared handles after each successful teardown before continuing.
- If persisted state has a handle for a selected driver but the rebuilt `DriverSet` cannot supply that driver, fail before registry removal so the handle remains available for retry or repair.
- If the simplified implementation sketch below conflicts with these hardening bullets, follow the hardening contract.
